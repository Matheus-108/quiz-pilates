...
import OrderBumpPopup from './order-bump-popup';
...
export default function TransparentCheckout() {
  ...
  const [totalValue, setTotalValue] = useState(2790); // R$27,90 in cents
  ...
  const handleGeneratePix = async (finalValue: number) => {
    ...
    try {
      const response = await fetch('/api/pix', {
        ...
        body: JSON.stringify({ value: finalValue, email }),
      });
